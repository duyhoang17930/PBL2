            ...
            if(event.type == Event::MouseButtonPressed){
           		if(event.mouseButton.button == Mouse::Left){
           			if(!BSan5.isMouseOver(ThemSan)){
           				San c(A.size()+1,"San5",100,true);
           				A.add(c);
						return 1;
					}
					if(!BSan7.isMouseOver(ThemSan)){
           				San c(A.size()+1,"San7",200,true);
           				A.add(c);
						return 1;
					}
				}
...
void GDSan(fstream& file,LinkedList<San> &A){
	const int Top=100;
	const int Bot=550;
        ...
        while (San.pollEvent(event)) {
            ...
            if(event.type == Event::MouseButtonPressed){
           		if(event.mouseButton.button == Mouse::Left){
					...
					if(!BSuaTrangThai.isMouseOver(San)){
						if(Selected!=0){
							A[Selected].setTT();
					...
					if(!BSuaThongTin.isMouseOver(San)){
						if(Selected!=0){
							A[Selected].suaThongTin();
							BSan[Selected].setText(A[Selected].thongTinSan2());
						}
					}
					if(!BDatSan.isMouseOver(San)){
						if(Selected!=0){
							GDDatSan();
						}
					}
...
void GDKhachHang(fstream& file2,LinkedList<KhachHang> &B){
	const int Top=100;
	const int Bot=550;
	const float Left =125;
    int Selected = 0;
	...
	while (KhachHangWin.isOpen()) {
        Event event;
        while (KhachHangWin.pollEvent(event)) {
            if (event.type == Event::Closed) {
            	ghiFile(file2,B);
                KhachHangWin.close(); // Ðóng c?a s? khi nh?n nút dóng
            }
            if(event.type == Event::MouseButtonPressed){
           		if(event.mouseButton.button == Mouse::Left){
           	 		if(!(BKH[B.size()].getTop()>Top && BKH[B.size()].getBot()<Bot)){
           	 			if(!BXuong.isMouseOver(KhachHangWin)){
           	 				for(int i=1;i<=B.size();i++){
					...
					if(!(BKH[1].getTop()>Top && BKH[1].getBot()<Bot)){
           	 			if(!BLen.isMouseOver(KhachHangWin)){
           	 				for(int i=1;i<=B.size();i++){
								BKH[i].setPosition({Left,float(BKH[i].getTop()+Bot-Top-50)});
							}
						}
           	 		...
           	 		for(int i=1;i<=B.size();i++){
           	 			if(!BKH[i].isMouseOver(KhachHangWin)){
           	 				if(Selected!=i){
					...
					if(!BXoaKH.isMouseOver(KhachHangWin)){
						if(Selected!=0){
					...
					if(!BThemKH.isMouseOver(KhachHangWin)){
						string ten,sdt,email;
						if(GDThemKH(a,ten,sdt,email)){
							KhachHang kh1(B.size()+1,ten,sdt,email);
							B.add(kh1);
					...
					if(!BSuaThongTinKH.isMouseOver(KhachHangWin)){
						if(Selected!=0){
							string ten,sdt,email;
							if(GDThemKH(a,ten,sdt,email)){
       	...
       	KhachHangWin.draw(WhiteBoard);
        BThemKH.drawTo(KhachHangWin);
		BXoaKH.drawTo(KhachHangWin);
		BLen.drawTo(KhachHangWin);
		BXuong.drawTo(KhachHangWin);
		BSuaThongTinKH.drawTo(KhachHangWin);
		BLichSuDatSan.drawTo(KhachHangWin);
		for(int i=1;i<=BKH.size();i++){
			if(BKH[i].getTop()>Top && BKH[i].getBot()<Bot){
				BKH[i].setIsAva(true);
				BKH[i].drawTo(KhachHangWin);
			}
			else BKH[i].setIsAva(false);
		}
        KhachHangWin.display();
    }
}
